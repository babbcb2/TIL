...
-- 9. 학번이 A517178 인 한아름 학생의 학점 총 평점을 구하는 SQL 문을 작성하시오. 단,
      이때 출력 화면의 헤더는 "평점" 이라고 찍히게 하고, 점수는 반올림하여 소수점 이하 한자리까지맊 표시한다.
...
SELECT  TO_NUMBER(SUBSTR(TERM_NO,1,4)) 년도, ROUND(AVG(POINT),1)
FROM    TB_GRADE
...
SELECT  TO_NUMBER(SUBSTR(TERM_NO,1,4)) AS "년도",
        TO_NUMBER(SUBSTR(TERM_NO,5,2)) AS "학기",
