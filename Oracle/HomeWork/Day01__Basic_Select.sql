-- Q1) 춘 기술대학교의 학과 이름과 계열을 표시하시오. 단, 출력 헤더는 "학과 명", "계열" 으로 표시하도록 한다.
SELECT  DEPARTMENT_NAME AS "학과이름",
        CATEGORY AS "계열"
FROM    TB_DEPARTMENT;


-- Q2) 학과의 학과 정원을 다음과 같은 형태로 화면에 출력핚다.
SELECT   DEPARTMENT_NAME||'의 정원은'||CAPACITY||'명 입니다' AS 학과별정원
FROM     TB_DEPARTMENT;


-- Q3) "국어국문학과" 에 다니는 여학생 중 현재 휴학중인 여학생을 찾아달라는 요청이 들어왔다.
...
-- Q4)도서관에서 대출 도서 장기 연체자 들을 찾아 이름을 게시하고자 한다. 
...
-- Q5) 입학정원이 20 명 이상 30 명 이하인 학과들의 학과 이름과 계열을 출력하시오.
...
-- Q6) 춘 기술대학교는 총장을 제외하고 모든 교수들이 소속 학과를 가지고 있다. 
...
-- Q7 혹시 전산상의 착오로 학과가 지정되어 있지 않은 학생이 있는지 확인하고자 핚다.
...
-- Q8 수강신청을 하려고 한다. 선수과목 여부를 확인해야 하는데, 
...
-- Q9 춘 대학에는 어떤 계열(CATEGORY)들이 있는지 조회해보시오.
SELECT  DISTINCT CATEGORY
FROM    TB_DEPARTMENT


-- Q10 02 학번 전주 거주자들의 모임을 만들려고 한다.
